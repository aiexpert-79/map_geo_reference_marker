...
# Function to convert raster data to GeoJSON format
def convert_raster_to_geojson(raster_path, output_geojson_path, category_data):
    ...
    for idx, (categoria, data) in enumerate(category_data.items(), start=1):
        ...
        poligonos_interiores = [Polygon(line) for line in merged_lines]
        gdf_poligonos_interiores = gpd.GeoDataFrame(geometry=poligonos_interiores, crs=src.crs)

        gdf_poligonos_interiores['OCUPACION'] = categoria
        gdf_poligonos_interiores.to_crs(epsg=25830)
        gdf_poligonos_interiores.to_file(output_path, driver="GeoJSON")
...
def main():
    ...
    raster_path = 'nuevo_raster'
    output_raster_path = 'raster_suavizado'
    output_geojson_path = 'FV_TRAMITACIÓN'
    shp_path = 'shp/FV_TRAMITACIÓN.shp'
    geojson_path = 'FV_TRAMITACIÓN.geojson'
